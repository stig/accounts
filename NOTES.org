...
** DONE Use constant-time comparison function for for hmac
** NEXT Ability to complete login
   ...
   - Compare the new HMAC to the one from the request. If it does not match,
     throw an error. Otherwise, it was a valid request, so let them in.

** TODO Set a session cookie when logged in & add protected endpoint to test it
** TODO Add a way to log out

   i.e. delete the session cookie.

** TODO Present form asking for the user to confirm that they want to log in

   Optionally, offer them to name the device they're logging in from.
